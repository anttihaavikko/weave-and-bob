...
func _picked(_body: Node2D):
	...
	if type == Type.Stomp:
		...
		GameState.show_texts("Downward dash!", "Press down to do a dash...", 0.5, 2.25)
	...
	if type == Type.Map:
		...
		if GameState.map_upgrades == 4:
			GameState.show_texts("Whole map visible!", "Tying off the loose ends...", 0.5, 2.5)
